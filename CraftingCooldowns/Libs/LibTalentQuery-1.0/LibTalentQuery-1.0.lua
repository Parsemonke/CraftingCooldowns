...
local MAJOR, MINOR = "LibTalentQuery-1.0", 190084
	...
	if (self.lastInspectName and self.lastInspectPending == 0) then
		...
		if (UnitGUID(self.lastInspectUnit) == self.lastInspectGUID) then
			...
			elseif (validateTrees) then
				-- Disabled on custom realms because localized or reordered custom talent trees
				-- can cause valid inspect data to be rejected before LibGroupTalents can read it.
			end
