...
function CooldownScanner:EnableScanner()
    ...
    -- FIX 1: Replaced SPELL_UPDATE_COOLDOWN with UNIT_SPELLCAST_SUCCEEDED.
    --        The old event fired every cooldown tick for every spell on the bar.
    --        The new event fires once, only when a cast actually completes.
    self:RegisterEvent("UNIT_SPELLCAST_SUCCEEDED", "OnSpellCastEvent")

    -- FIX 2: BAG_UPDATE now only sets a dirty flag instead of scheduling a
    --         full scan immediately. The bag is rebuilt lazily on next scan.
...
-- FIX 2: Mark dirty; do not schedule a scan here.
